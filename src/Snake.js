...
export default class Snake extends Component {
  ...
  drawFood = () => {
    ...
    this.canvas.arc(x, y, r, 0, Math.PI * 2, true);
    this.canvas.fill();
  };
  ...
  moveSnakeForward = () => {
    ...
    const snake = this.state.snake.slice(0, this.state.snake.length - 1);
    ...
    if (newHeadCoordinate[0] < 0 || newHeadCoordinate[1] < 0 ||
      newHeadCoordinate[0] > canvasWidth || newHeadCoordinate[1] > canvasHeight ) {
      this.setState({ ...this.state, gameEnded: true });
    } else
    ...
    if (JSON.stringify(snakeHead) === JSON.stringify(this.state.foodCoordinate)) {
      this.setState({
        ...
        snake: [newHeadCoordinate, ...this.state.snake]
      });
    } else {
      ...
      this.setState({ ...this.state, snake: [newHeadCoordinate, ...snake] });
    }
  };
  ...
  handleKeyDown = (event) => {
    ...
    this.setState({ ...this.state, direction: direction || this.state.direction });
  };

  focusDirectionInput = () => {
    this.refs.directionInput.focus();
  };
  ...
  render() {
    return (
      <div>
        <canvas
          ref="canvas"
          onClick={ this.focusDirectionInput }
        ...
        { this.state.gameEnded &&
          <div id="infoWrapper" className="infoWrapper">
            <div id="playAgain" className="info" onClick={ this.init }>Play again!</div>
            <div id="score" className="info">SCORE: { this.state.snake.length - 3 } </div>
          </div>
        }
      </div>
    );
